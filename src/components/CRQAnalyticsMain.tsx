...
<Modal open={open} onClose={onClose} keepMounted={false}>
      ...
      >
        ...
        >
          ...
          <Box>
            <Typography
              sx={{ fontSize: 13, fontWeight: 700, color: isDark ? "#e2e8f0" : "#1e293b" }}
            >
              {cfg.title}
            </Typography>
            <Typography
              sx={{ fontSize: 10, color: isDark ? "#64748b" : "#94a3b8", mt: 0.25 }}
            >
              {rows.length} records
            </Typography>
          </Box>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            <IconButton
              ...
              sx={{
                bgcolor: isDark ? "rgba(74,222,128,0.10)" : "rgba(34,197,94,0.08)",
                ...
                height: 30,
                "&:hover": {
                  bgcolor: isDark ? "rgba(74,222,128,0.22)" : "rgba(34,197,94,0.15)",
                },
              }}
            >
              ...
              {rows.map((row, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell sx={bodyCellSx}>
                      <Typography
                        sx={{ fontSize: 11, fontWeight: 700, color: isDark ? "#e2e8f0" : "#1e293b" }}
                      >
                        {row.aging}
                      </Typography>
                    </TableCell>
                    ...
                    <TableCell
                      sx={{ ...bodyCellSx, fontSize: 10, color: isDark ? "#94a3b8" : "#64748b" }}
                    >
                      {row.binGroup}
                    </TableCell>
        ...
        >
          <Typography sx={{ fontSize: 10, color: isDark ? "#475569" : "#94a3b8" }}>
        ...
        {AGING_HEATMAP_DATA.map((row) => (
          <Box key={row.bucket} sx={{ display: "grid", gridTemplateColumns: "130px 1fr 1fr", gap: 1, alignItems: "center" }}>
            ...
            {[{ val: row.ccb, key: "ccb" }, { val: row.se, key: "se" }].map(({ val, key }) => {
              ...
              return (
                <Box key={key} sx={{ bgcolor: bg, borderRadius: 1.5, py: 1, textAlign: "center", transition: "opacity 0.2s", "&:hover": { opacity: 0.85 } }}>
                  <Typography sx={{ fontSize: 13, fontWeight: 700, color: "#fff" }}>{val.toLocaleString()}</Typography>
                </Box>
              );
            })}
          </Box>
...
const quickFilterBtnSx = (active: boolean, isDark: boolean, muted: string): SxProps<Theme> => ({
  background: active
...
const CRQAnalyticsMain: React.FC = () => {
  ...
  const [quickDateFilter, setQuickDateFilter] = useState<QuickDateFilter>("30d");
  ...
  const kpiValues = useMemo(() => {
    ...
    const src = isCCB ? MOCK_KPI_CCB : null;
    return {
      total:    { value: src ? src[0].value : 120,   sub: "All requests", trend: src ? src[0].trend : 8   },
      open:     { value: src ? src[1].value : 48,    sub: "In progress",  trend: src ? src[1].trend : -3  },
      closed:   { value: src ? src[2].value : 62,    sub: "Completed",    trend: src ? src[2].trend : 12  },
      rejected: { value: src ? src[3].value : 10,    sub: "Declined",     trend: src ? src[3].trend : -2  },
      sla:      { value: src ? src[4].value : "87%", sub: "On-time rate", trend: src ? src[4].trend : 1.2 },
    };
  }, [data, isCCB]);

  // ── Filter-aware open-CRQ domain data ────────────────────────────────────
  const openCrqDomainData = isCCB ? OPEN_CRQ_DOMAIN_DATA_CCB : OPEN_CRQ_DOMAIN_DATA;
  ...
  {[{ color: "#6366f1", label: "CCB" }, { color: "#94a3b8", label: "SE" }].map(({ color, label }) => {
    ...
    const isActive = activeLegend === label || activeLegend === null;
    ...
    return (
      ...
      >
        ...
        <Typography sx={{ fontSize: 10, color: muted }}>{label}</Typography>
            ...
            <Box sx={{ height: 240 }}>
              <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={openCrqDomainData} margin={{ top: 4, right: 8, bottom: 24, left: -20 }}>
                  
                  <CartesianGrid strokeDasharray="3 3" stroke={gridColor} />
                  <XAxis dataKey="domain" tick={{ fontSize: 11, fill: tickColor }} axisLine={false} tickLine={false} angle={-35} textAnchor="end" height={48} interval={0} />
          ...
          <ChartCard title="RUN RATE: CCB TO SE" isDark={isDark} height={240} isLoading={isFetching} onTitleClick={() => openPopup("runrate")}>
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={runRate} margin={{ top: 8, right: 12, bottom: 0, left: -20 }}>
                ...
                <CartesianGrid strokeDasharray="3 3" stroke={gridColor} />
                <XAxis dataKey="date" tick={{ fontSize: 8, fill: tickColor }} axisLine={false} tickLine={false} interval={3} angle={-35} textAnchor="end" height={36} />
                <YAxis tick={{ fontSize: 9, fill: tickColor }} axisLine={false} tickLine={false} domain={[0, 10]} ticks={[0, 2, 4, 6, 8, 10]} />
                <RTooltip contentStyle={tooltipStyle} />
                <Legend wrapperStyle={{ fontSize: 10, paddingTop: 4 }} formatter={(v) => <span style={{ color: tickColor }}>{v}</span>} />
                <Bar dataKey="receivedInCCB" name="Received in CCB" fill="url(#grCCB)" radius={[3, 3, 0, 0]} maxBarSize={14} />
                <Line type="monotone" dataKey="movedToSE" name="Moved to SE" stroke="#f97316" strokeWidth={2} dot={{ r: 3, fill: "#f97316", strokeWidth: 0 }} activeDot={{ r: 5 }} />
                <Line type="monotone" dataKey="seToClosed" name="SE to Closed" stroke="#94a3b8" strokeWidth={2} dot={{ r: 3, fill: "#94a3b8", strokeWidth: 0 }} activeDot={{ r: 5 }} />
              </ComposedChart>
            </ResponsiveContainer>
          </ChartCard>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr 1fr" }, gap: 1.5, mb: 1.5 }}>
          ...
          <Box sx={panelSx(isDark)}>
            ...
            <Box sx={{ height: 220 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={TOP4_REJECTION_REASONS} layout="vertical" margin={{ top: 4, right: 56, bottom: 0, left: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke={gridColor} horizontal={false} />
                  ...
                  <RTooltip
                    ...
                    formatter={(v: number, _name: string, props: any) => [`${v} CRQs (${props.payload.pct}%)`, "Count"]}
                    labelFormatter={(label) => label}
                  />
                  <Bar dataKey="count" radius={[0, 4, 4, 0]} maxBarSize={28} label={{ position: "right", fontSize: 10, fill: muted, formatter: (v: number) => v }}>
                    {TOP4_REJECTION_REASONS.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </Box>
            ...
            <Box sx={{ height: 190, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  >
                    {rejectionPieData.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                  </Pie>
                  <RTooltip
                    contentStyle={tooltipStyle}
